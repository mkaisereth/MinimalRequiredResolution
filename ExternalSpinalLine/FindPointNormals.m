...
l_mins = zeros(size(points,1),1);
l_maxs = zeros(size(points,1),1);
dets = zeros(size(points,1),1);
for i = 1:(size(points,1))
    ...
    dets(i) = det(Cmat);
    d = diag(d);
    [lambda_min,k_min] = min(d);
    [lambda_max,k_max] = max(d);
    ...
    l_mins(i) = lambda_min;
    l_maxs(i) = lambda_max;
    curvature(i) = lambda_min * lambda_max;% / sum(d);
end
%% flipping normals
